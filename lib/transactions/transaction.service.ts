...
type BulkUpdateReportingInput = {
    ...
    filters?: Pick<TransactionQuery, "search" | "paymentStatus" | "reportingStatus">;
    tahap2Date?: string;
    tahap3Date?: string;
};
...
function mapMidtransToOrderStatus(status?: string, fraudStatus?: string): OrderStatus {
...
async function syncPendingOrderFromMidtrans(orderId: string): Promise<boolean> {
    ...
    try {
        statusResponse = (await coreApi.transaction.status(orderId)) as MidtransStatusResponse;
    } catch (error) {
...
function mapUiToOrderStatus(status?: string): OrderStatus | null {
  ...
  documentation-customer-reporting-sync
function buildOrderWhereInput(query: Pick<TransactionQuery, "search" | "paymentStatus">): Prisma.OrderWhereInput {
...
export async function getDashboardTransactionMetrics(): Promise<DashboardTransactionMetrics> {
    ...
    const tanpaDokumentasi = orders.filter(
        (order) => getEffectiveReports(order.reports, order.group?.reports).length === 0,
    ).length;
...
export async function getDashboardTransactionMetrics(): Promise<DashboardTransactionMetrics> {
  ...
  for (const order of orders) {
    ...
    const videoUrl = combinedDocs.find((doc) => doc.mediaType === "VIDEO")?.mediaUrl ?? null;
...
export async function updateTransactionDocumentations(input: UpdateTransactionDocumentationInput) {
...
export async function bulkUpdateReportingStatus(input: BulkUpdateReportingInput) {
    ...
    if (input.applyToFiltered) {
        ...
        uniqueOrderIds = orders
            .filter((order) => {
                if (!filters.reportingStatus || filters.reportingStatus === "Semua Pelaporan") {
                    return true;
                }
        ...
        uniqueOrderIds = Array.from(new Set((input.orderIds ?? []).filter(Boolean)));
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
        ...
        const groupIds = Array.from(
            new Set(existingOrders.map((order) => order.animalGroupId).filter(Boolean)),
        ) as string[];
...
export async function distributeDocumentations(input: DistributeDocumentationInput) {
    ...
    return prisma.$transaction(async (tx) => {
        ...
        if (photoItems.length > 0 && photoDonorFilter) {
            ...
            if (photoTargetOrders.length === 0) {
                throw new Error("Tidak ada customer yang cocok dengan filter nama pequrban.");
            }

            const groupIds = Array.from(
                new Set(photoTargetOrders.map((order) => order.animalGroupId).filter(Boolean)),
            ) as string[];
        ...
        if (videoItem || shouldRemoveVideo) {
            const yearWhere = input.distributionYear
                ? (() => {
                    const { startDate, endDate } = getYearRange(input.distributionYear as number);
                    return {
                        gte: startDate,
                        lt: endDate,
                    };
            ...
            if (allCustomerOrders.length === 0) {
                throw new Error("Belum ada transaksi customer untuk distribusi video.");
            }

            const groupIds = Array.from(
                new Set(allCustomerOrders.map((order) => order.animalGroupId).filter(Boolean)),
            ) as string[];
...
export async function uploadSlaughterDocumentationByFolders(input: SlaughterFolderUploadInput) {
    ...
    if (validPhotos.length === 0) {
        throw new Error("File foto dalam folder tidak ditemukan atau format tidak valid.");
    }

    const groupedByFolder = new Map<string, { originalFolderName: string; photos: SlaughterFolderPhotoInput[] }>();
        ...
        for (const [folderKey, folderGroup] of groupedByFolder.entries()) {
            ...
            if (matchedByName.length > 1) {
                ...
                const matchedByOrd = matchedByName.filter((order) => {
                    const invoiceNumber =
                        order.invoice?.invoiceNumber ?? `ORD-${order.id.slice(0, 8).toUpperCase()}`;
...
export async function listDocumentationDistributionYears(query: DocumentationYearsQuery) {
    const page = Math.max(1, query.page ?? 1);
    const pageSize = Math.min(100, Math.max(1, query.pageSize ?? 10));
    ...
    const data = await Promise.all(
        pagedYears.map(async (year) => {
            ...
            return {
                year,
                uploadDate: latestVideoDoc ? formatLongDate(latestVideoDoc.createdAt) : "",
                status: latestVideoDoc ? ("Tersedia" as const) : ("Belum Tersedia" as const),
            };
        }),
    );
