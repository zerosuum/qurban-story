...
type EditProdukModalProps = {
    isOpen: boolean;
    onClose: () => void;
    initialData: TambahProdukForm;
    initialImageUrls?: string[];
    onSubmitProduct?: (formData: TambahProdukForm, imageFiles: File[], existingImageUrls: string[]) => Promise<void> | void;
...
export default function EditProdukModal({
    ...
    onSubmitProduct,
    onSubmitResult,
}: EditProdukModalProps) {
    const [form, setForm] = useState<TambahProdukForm>(initialData);
    ...
    useEffect(() => {
        ...
        setForm(initialData);
        setSelectedImages(initialImageUrls.map((url) => ({ file: null, previewUrl: url })));
    }, [initialData, initialImageUrls, isOpen]);
        ...
        try {
            if (onSubmitProduct) {
                await onSubmitProduct(
                    form,
                    selectedImages.map((item) => item.file).filter((item): item is File => item instanceof File),
                    selectedImages
    ...
    return (
        <>
            {!isConfirmOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-neutral-800/50 p-4 backdrop-blur-sm">
                    <div className="relative h-[896px] w-[600px] max-h-[calc(100vh-2rem)] max-w-[calc(100vw-2rem)] overflow-y-auto rounded-xl border border-neutral-100 bg-white p-6 shadow-[0_8px_28px_-6px_rgba(24,39,75,0.12),0_18px_88px_-4px_rgba(24,39,75,0.14)] [scrollbar-width:thin] [scrollbar-color:#044B57_#F3F3F3] [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:rounded-full [&::-webkit-scrollbar-track]:bg-neutral-50 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-primary-100 [&::-webkit-scrollbar-thumb:hover]:bg-primary-300">
                        ...
                        <h4 className="mb-4 text-2xl font-bold text-black">Edit Produk</h4>
                        ...
                        <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
                            ...
                            {selectedImages.length > 0 && (
                                <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
                                    {selectedImages.map((item, index) => (
                                        <div key={`${item.previewUrl}-${index}`} className="relative overflow-hidden rounded-xl border border-neutral-100">
                            ...
                            <div className="flex flex-col gap-2">
                                <label htmlFor="edit-namaProduk" className="text-base font-semibold text-neutral-500">
                            ...
                            <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                                <div className="flex flex-col gap-2">
                                    <label htmlFor="edit-tipe" className="text-base font-semibold text-neutral-500">Tipe <span className="text-red-500">*</span></label>
                                    <select
                                        id="edit-tipe"
                                        value={form.tipe}
                                ...
                                <div className="flex flex-col gap-2">
                                    <label htmlFor="edit-berat" className="text-base font-semibold text-neutral-500">Berat <span className="text-red-500">*</span></label>
                                    <input
                                        id="edit-berat"
                                        value={form.berat}
                                ...
                                <div className="flex flex-col gap-2">
                                    <label htmlFor="edit-stok" className="text-base font-semibold text-neutral-500">Stok <span className="text-red-500">*</span></label>
                                    <input
                                        id="edit-stok"
                                        value={form.stok}
                            ...
                            <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                                <div className="flex flex-col gap-2">
                                    <label htmlFor="edit-harga" className="text-base font-semibold text-neutral-500">Harga (Rp) <span className="text-red-500">*</span></label>
                                    <input
                                        id="edit-harga"
                                        value={form.harga}
                                ...
                                <div className="flex flex-col gap-2">
                                    <label htmlFor="edit-diskon" className="text-base font-semibold text-neutral-500">Diskon</label>
                                    <input
                                        id="edit-diskon"
                                        value={form.diskon}
                            ...
                            <div className="flex flex-col gap-2">
                                <label htmlFor="edit-deskripsi" className="text-base font-semibold text-neutral-500">Deskripsi</label>
                                <textarea
                                    id="edit-deskripsi"
            ...
            <ConfirmationPopup
                ...
                title="Konfirmasi"
                message="Apakah Anda yakin ingin menyimpan perubahan produk ini?"
